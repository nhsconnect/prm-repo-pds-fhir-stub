...
resource "aws_alb" "alb-internal" {
  ...
  security_groups = [
    aws_security_group.pds_fhir_stub_alb.id,
    aws_security_group.alb_to_pds_fhir_stub_ecs.id,
    aws_security_group.service_to_pds_fhir_stub.id,
    aws_security_group.vpn_to_pds_fhir_stub.id,
    aws_security_group.gocd_to_pds_fhir_stub.id
  ]
  internal        = true
...
resource "aws_security_group" "pds_fhir_stub_alb" {
  name        = "${var.environment}-alb-${var.component_name}"
  description = "pds-fhir-stub ALB security group"
  vpc_id      = data.aws_ssm_parameter.deductions_private_vpc_id.value

  ingress {
    description     = "Allow traffic from pds adaptor to ALB"
    protocol        = "tcp"
    from_port       = 443
    to_port         = 443
    security_groups = [
      data.aws_ssm_parameter.pds-adaptor-ecs-sg-id.value
    ]
  }
...
resource "aws_alb_listener" "int-alb-listener-https" {
  ...
  certificate_arn = aws_acm_certificate_validation.pds-fhir-stub-cert-validation.certificate_arn
...
resource "aws_lb_listener_certificate" "pds-fhir-stub-int-listener-cert" {
  listener_arn    = aws_alb_listener.int-alb-listener-https.arn
  certificate_arn = aws_acm_certificate_validation.pds-fhir-stub-cert-validation.certificate_arn
}

resource "aws_security_group" "alb_to_pds_fhir_stub_ecs" {
  name        = "${var.environment}-alb-to-${var.component_name}-ecr"
  description = "Allows pds fhir stub ALB connections to pds-fhir-stub component task"
  vpc_id      = data.aws_ssm_parameter.deductions_private_vpc_id.value

  egress {
    description = "Allow outbound connections to pds-fhir-stub ECS Task"
    from_port   = 0
    to_port     = 0
    protocol    = "-1"
...
resource "aws_security_group" "service_to_pds_fhir_stub" {
  name        = "${var.environment}-service-to-${var.component_name}"
  description = "Controls access from other services to pds-fhir-stub"
  vpc_id      = data.aws_ssm_parameter.deductions_private_vpc_id.value
...
resource "aws_ssm_parameter" "service_to_pds_fhir_stub" {
  name = "/repo/${var.environment}/output/${var.repo_name}/service-to-pds-fhir-stub-sg-id"
  type = "String"
  value = aws_security_group.service_to_pds_fhir_stub.id
  tags = {
    CreatedBy   = var.repo_name
    Environment = var.environment
  }
...
resource "aws_security_group" "vpn_to_pds_fhir_stub" {
  name        = "${var.environment}-vpn-to-${var.component_name}"
  description = "Controls access from vpn to pds-fhir-stub"
  vpc_id      = data.aws_ssm_parameter.deductions_private_vpc_id.value
...
resource "aws_security_group_rule" "vpn_to_pds_fhir_stub" {
  count       = var.grant_access_through_vpn ? 1 : 0
  type        = "ingress"
  description = "Allow vpn to access PDS fhir stub ALB"
  protocol    = "tcp"
  from_port   = 443
  to_port     = 443
  source_security_group_id = data.aws_ssm_parameter.vpn_sg_id.value
  security_group_id = aws_security_group.vpn_to_pds_fhir_stub.id
}

resource "aws_security_group" "gocd_to_pds_fhir_stub" {
  name        = "${var.environment}-gocd-to-${var.component_name}"
  description = "Controls access from gocd to pds-fhir-stub"
  vpc_id      = data.aws_ssm_parameter.deductions_private_vpc_id.value

  ingress {
    description = "Allow gocd to access pds-fhir-stub ALB"
    protocol    = "tcp"
    from_port   = 443
    to_port     = 443
